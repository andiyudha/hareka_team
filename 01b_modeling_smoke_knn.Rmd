...
The train function streamlines the model building and evaluation process. Using k-nearest neighbours (KNN) model

For knn, there is only one tuning parameter, k. I have three options for tuning:
...
plot(m_knn, main = "KNN 10-fold Cross-Validation: No specification on tuning parameter")
```


```{r}
plot(m_knn_2, main = "KNN 10-fold Cross-Validation: tuneGrid")
```
```{r}

plot(m_knn_3, main = "KNN 10-fold Cross-Validation: Try 10 random parameters")

```

based on running the KNN model find that Model with Tuning Grid is high Accuracy with 0.9996607.
...
predict_knn2 <- predict(m_knn_2, newdata = test_data)

# Confusion Matrix
tbl_knn2 <- confusionMatrix(predict_knn2, test_data$fire_alarm)

tbl_knn2
```
```{r}

importance_2 <- varImp(m_knn_2, scale=FALSE)
plot(importance_2, main = "Importance Variable in Smoke Detection")
...
```{r}
predict_knn <- predict(m_knn, newdata = test_data)

# Confusion Matrix
tbl_knn <- confusionMatrix(predict_knn, test_data$fire_alarm)

tbl_knn
```

```{r}
predict_knn3 <- predict(m_knn_3, newdata = test_data)

# Confusion Matrix
tbl_knn3 <- confusionMatrix(predict_knn3, test_data$fire_alarm)

tbl_knn3
```

Based on 3 options, found that Option number 2 with tuneGrid: specify the numbers to try, seq(2, 20, 1), a sequence of number from 2 to 20 with an interval of 1 has high accuracy with 0.9997.

```{r}
test_data %>%
  mutate(
    predict = predict_knn2
  ) %>% glimpse() -> test_predict #dataset with prediction
```
...
# write testing data into to database
