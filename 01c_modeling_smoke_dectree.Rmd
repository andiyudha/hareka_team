...
# Option 2: Try expand grid
# 
grid <- expand.grid( .winnow = c(TRUE,FALSE), .trials=c(1,5,10,15,20), .model="tree" )

m_c5_2 <- train(form = fire_alarm~.,
               data = train_data,
               method = 'C5.0',
...
plot(m_c5, main = "Decision Tree with 10-fold Cross-Validation: No specification on tuning parameter")
```


```{r}
plot(m_c5_2, main = "Decision Tree with 10-fold Cross-Validation: with tuneGrid")
```

based on running the Decision Tree model find that Model with Tuning Grid is high Accuracy with 0.9997805.
...
predict_c52 <- predict(m_c5_2, newdata = test_data)

# Confusion Matrix
tbl_c52 <- confusionMatrix(predict_c52, test_data$fire_alarm)

tbl_c52
```
```{r}
predict_c5 <- predict(m_c5, newdata = test_data)

# Confusion Matrix
tbl_c5 <- confusionMatrix(predict_c5, test_data$fire_alarm)

tbl_c5
```


Based on 2 options, found that Option number 1 with tuneGrid has high accuracy with 0.9998.

```{r}

importance_c5 <- varImp(m_c5, scale=FALSE)
plot(importance_c5, main = "Importance Variable in Smoke Detection with Decision Tree")
```
...
saveRDS(m_c5, "./dectree_model.rds")
```
